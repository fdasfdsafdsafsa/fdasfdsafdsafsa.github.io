...
panel_install(){
    ...
    sed "s/utf8mb4=uca1400_ai_ci/utf8mb4=utf8mb4_general_ci/g" -i /etc/mysql/mariadb.conf.d/50-server.cnf && service mariadb restart && tar -xzvf panel.tar.gz
...
read -p "Введите имя пользователя администратора: " USERNAME
read -p "Введите имя администратора: " FIRSTNAME
read -p "Введите фамилию администратора: " LASTNAME
read -s -p "Введите пароль администратора: " PASSWORD
echo
read -p "Установить Wings (true/false): " WINGS


if [ -z "$FQDN" ] || [ -z "$SSL" ] || [ -z "$EMAIL" ] || [ -z "$USERNAME" ] || [ -z "$FIRSTNAME" ] || [ -z "$LASTNAME" ] || [ -z "$PASSWORD" ] || [ -z "$WINGS" ]; then
    echo "Error! Использование скрипта неверно."
    exit 1
fi
